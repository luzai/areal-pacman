...
class PacmanAgentConfig(PPOConfig):
    ...
    reward_objective_contract: str = field(
        ...
        metadata={
            "help": (
                "Reward/advantage normalization contract. step_local_raw_v1 "
                "keeps dense per-decision Pacman rewards uncentered so reward "
                "signs remain state-local rather than becoming a global "
                "action-token baseline."
            )
        },
    )
