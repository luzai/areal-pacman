...
git -C pacman-python checkout a492347f40cd871a3cf82b695346cffc4a5f4077
```
...
下面是历史 8×H100 gate 已验证的关键环境版本，可作为锁定环境的参考：
...
# 使用上方记录的 8×H100 gate 已验证版本。
# PyTorch wheel 必须兼容服务器 CUDA；该 gate 使用 torch 2.11.0+cu130。
...
```text
初始化：Curriculum 1 = Qwen/Qwen3.5-9B；Curriculum 2 = Curriculum 1 checkpoint
GPU：8
训练 episode：8
验证 episode：2
batch size：4
...
top_p：1.0
thinking：false
Edward options：true（`open_action_mask=false`）
```

## Reward v3

Reward 的唯一准确信息源是
[`areal_pacman/level1/rewards.py`](areal_pacman/level1/rewards.py) 和所选 YAML。
README 不复制容易随配方变动而漂移的奖励公式。
...
2026-09-04 的上一版发布基线曾在 `H100_2_1` 对应的 8×NVIDIA H800 节点完成
无 GPU 的 headless smoke；它不等同于本次两阶段 curriculum 的分布式训练验证。
本次发布的实际测试结果应以当前 release commit 和运行 manifest 为准。
...
以上范围不足以完全复现训练。下面是 2026-07-30 从 8×H100 gate 实际
...
安装与该 gate 一致的 Python/GPU 包。PyTorch wheel 必须是 CUDA 13.0
