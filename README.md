...
- [AReaL Pacman RL Recipe Design](AREAL_RECIPE_DESIGN.md)
...
```powershell
python scripts\prepare_level1_dataset.py `
...
export CONFIG="$PWD/configs/level1_live_state_step32_16update_group12_8gpu.yaml"
...
areal-pacman repository (this repository)
  ...
  configs/
    Reproducible AReaL experiment configurations.
  scripts/run_level1_training.sh
    Validated Linux/H100 training entry point.
  scripts/evaluate_level1.py
    Independent checkpoint evaluator.
  scripts/summarize_level1_trajectories.py
...
the RL recipe, and official AReaL owns distributed training. Do not copy the
game implementation into this package or run production Level-1 through the
historical `areal_pacman.env.PacmanEnv`.
...
`scripts/run_level1_training.sh` resolve `areal` from the clean official
...
`areal_pacman.workflow.PacmanNativeVisionWorkflow` now implements the
...
`maapacman.env.PygamePacmanEnv`. The separate `areal_pacman.env.PacmanEnv`
...
- [areal_pacman/env.py](areal_pacman/env.py): deterministic text PacMan environment.
- [areal_pacman/baselines.py](areal_pacman/baselines.py): random 和 greedy baseline agents.
- [areal_pacman/evaluate.py](areal_pacman/evaluate.py): episode evaluator.
- [areal_pacman/dataset.py](areal_pacman/dataset.py): JSONL prompt dataset generator.
- [areal_pacman/maze_suite.py](areal_pacman/maze_suite.py): `multi_maze_v1` manifest loader, split/hash validation, and registry.
- [multi_maze_v1.json](areal_pacman/maze_suites/multi_maze_v1.json): deterministic `64/16/32` train/validation/test maze manifest with oracle solutions.
...
- [remote_supervise_multimaze_v1.sh](scripts/remote_supervise_multimaze_v1.sh): GPU-empty-gated base evaluation, multi-maze RL, complete checkpoint, and held-out evaluation pipeline.
- [remote_supervise_multimaze_safeprogress_v1.sh](scripts/remote_supervise_multimaze_safeprogress_v1.sh): alpha-4 safe-progress GRPO, per-epoch validation selection, and selected-checkpoint train/test evaluation.
- [config_pacman.yaml](config_pacman.yaml): minimal local-scheduler AReaL config.
...
- [config_text_rollout.yaml](config_text_rollout.yaml): minimal pure text episode rollout smoke.
- [config_text_rollout_longer.yaml](config_text_rollout_longer.yaml): slightly larger pure text rollout, for short reward and trajectory checks.
- [config_text_rollout_1p5b.yaml](config_text_rollout_1p5b.yaml): same rollout as longer, but with `Qwen/Qwen2.5-1.5B-Instruct`.
- [config_text_rollout_1p5b_legal.yaml](config_text_rollout_1p5b_legal.yaml): 1.5B legal-action prompt and illegal-action penalty ablation.
- [config_text_rollout_1p5b_rules.yaml](config_text_rollout_1p5b_rules.yaml): 1.5B explicit game-rules prompt PPO baseline.
- [config_text_rollout_1p5b_grpo.yaml](config_text_rollout_1p5b_grpo.yaml): 1.5B GRPO-style config with `n_samples=4` and group reward normalization.
- [config_text_rollout_1p5b_overfit_grpo.yaml](config_text_rollout_1p5b_overfit_grpo.yaml): 1.5B 4-episode overfit gate with `prompt_style=ghost_legal`.
- [config_text_rollout_1p5b_tiny_overfit_grpo.yaml](config_text_rollout_1p5b_tiny_overfit_grpo.yaml): 1.5B tiny sparse curriculum overfit gate, no teacher hint.
- [config_text_rollout_1p5b_teacher_overfit_grpo.yaml](config_text_rollout_1p5b_teacher_overfit_grpo.yaml): 1.5B teacher-hint overfit warm-start gate.
- [config_text_rollout_1p5b_teacher_pure_overfit_grpo.yaml](config_text_rollout_1p5b_teacher_pure_overfit_grpo.yaml): 1.5B pure hint-only overfit gate, no execution-time teacher override.
- [config_text_rollout_1p5b_long_debug_grpo.yaml](config_text_rollout_1p5b_long_debug_grpo.yaml): larger debug config to use only after the overfit gate passes.
- [config_text_rollout_7b_overfit_grpo.yaml](config_text_rollout_7b_overfit_grpo.yaml): 7B default-map sparse overfit config, single-GPU attempt OOMed during optimizer state allocation.
- [config_text_rollout_7b_overfit_grpo_8gpu.yaml](config_text_rollout_7b_overfit_grpo_8gpu.yaml): 7B default-map sparse overfit config using 8 GPUs with `vllm:d4p1t1` rollout and `fsdp:d4p1t1` actor.
- [config_text_rollout_7b_medium_default_overfit_grpo_8gpu.yaml](config_text_rollout_7b_medium_default_overfit_grpo_8gpu.yaml): 7B medium-map sparse overfit config using `layout_name=medium_default` and `prompt_style=ghost_legal_strict`.
- [config_text_rollout_7b_medium_default_reason_debug_8gpu.yaml](config_text_rollout_7b_medium_default_reason_debug_8gpu.yaml): 7B medium-map debug config using `prompt_style=ghost_legal_reason` and `max_new_tokens=96` so trajectories can include short reasons plus `Action: <token>`.
- [config_text_rollout_qwen3p5_9b_json_medium_overfit_grpo_6epoch_8gpu.yaml](config_text_rollout_qwen3p5_9b_json_medium_overfit_grpo_6epoch_8gpu.yaml): Qwen3.5-9B strict no-BFS `ghost_legal_json` 6epoch overfit config using the current `2/12` no-training baseline setting.
- [config_text_rollout_qwen3p5_9b_jsonfirst_medium_overfit_grpo_6epoch_1024_8gpu.yaml](config_text_rollout_qwen3p5_9b_jsonfirst_medium_overfit_grpo_6epoch_1024_8gpu.yaml): Qwen3.5-9B no-BFS `ghost_legal_json_first` 6epoch overfit config, using JSON-first prompt and `max_new_tokens=1024` to test whether the `256` run was bottlenecked by verbose pre-JSON output.
- [config_text_rollout_qwen3p5_9b_jsonfirst_medium_overfit_grpo_6epoch_256_8gpu.yaml](config_text_rollout_qwen3p5_9b_jsonfirst_medium_overfit_grpo_6epoch_256_8gpu.yaml): Qwen3.5-9B no-BFS JSON-first short-output diagnostic config.
- [config_text_rollout_qwen3p5_9b_actiononly_medium_overfit_grpo_6epoch_8tok_8gpu.yaml](config_text_rollout_qwen3p5_9b_actiononly_medium_overfit_grpo_6epoch_8tok_8gpu.yaml): Qwen3.5-9B no-BFS action-only diagnostic config with `max_new_tokens=8`.
- [config_text_rollout_qwen3p5_9b_actiononly_guided_medium_overfit_grpo_6epoch_8192_8gpu.yaml](config_text_rollout_qwen3p5_9b_actiononly_guided_medium_overfit_grpo_6epoch_8192_8gpu.yaml): Qwen3.5-9B no-BFS action-only guided-choice overfit config with `max_new_tokens=8192`.
- [config_text_rollout_qwen3p5_9b_actiononly_medium_overfit_grpo_6epoch_8192_parserfix_8gpu.yaml](config_text_rollout_qwen3p5_9b_actiononly_medium_overfit_grpo_6epoch_8192_parserfix_8gpu.yaml): Qwen3.5-9B no-BFS action-only parser-fix config with `max_new_tokens=8192`.
- [level1_image_anticollapse_4update_group12_8gpu.yaml](configs/level1_image_anticollapse_4update_group12_8gpu.yaml): production original-pygame image-only gate with group size 12, exactly four updates, temperature `0.7`, LR `1.5e-6`, KL `0.01`, and separate greedy validation.
- [level1_image_progress_4update_group12_8gpu.yaml](configs/level1_image_progress_4update_group12_8gpu.yaml): isolated nearest-pellet alpha-1 follow-up with matched sampled12 in-training validation and separately reported greedy1 validation.
- [level1_live_state_4update_group12_8gpu.yaml](configs/level1_live_state_4update_group12_8gpu.yaml): live-state follow-up with `open_action_mask: true`; every model request is dynamically constrained to the current engine-reported open movement actions, and [areal_pacman_action_logprobs.patch](patches/areal_pacman_action_logprobs.patch) applies the identical mask and temperature to actor/reference log-probabilities. This config uses `top_p=1.0` and vLLM `processed_logprobs` so cached old and recomputed new probabilities describe the same policy.
- [evaluate_level1.py](scripts/evaluate_level1.py): thinking-disabled greedy or sampled level-1 evaluator with independent decoding, prompt selection, parallel episodes, and collision-free trajectories.
- [reevaluate_level1_group12.sh](scripts/reevaluate_level1_group12.sh): corrected base/all-checkpoint greedy evaluation plus 24 matched sampled final episodes.
- [run_level1_prompt_ab.sh](scripts/run_level1_prompt_ab.sh): base/final `minimal_v1` versus static live-inspired image-only prompt A/B.
- [summarize_level1_trajectories.py](scripts/summarize_level1_trajectories.py): collision-free chronological train/validation batch audit.
- [evaluate_level1_run.sh](scripts/evaluate_level1_run.sh): sequential one-GPU dual validation for base plus all four updates: greedy1 at `0/1.0` and sampled12 at the training `0.7/0.95` decoding.
- [compare_level1_run.py](scripts/compare_level1_run.py): deterministic dual-validation report builder with separate sampled-primary and greedy checkpoint labels.
- [smoke_tms_offload.py](scripts/smoke_tms_offload.py): H100 torch-memory-saver pause/resume integration smoke.
...
python3 -m pytest -q
python3 -m areal_pacman.evaluate --agent greedy --episodes 20
python3 -m areal_pacman.dataset --output run_artifacts/pacman_prompts.jsonl --episodes 5
python3 -m areal_pacman.dataset --mode episode --output run_artifacts/pacman_episode_specs.jsonl --episodes 5
```
...
```bash
PACMAN_TRAJECTORY_DIR=run_artifacts/trajectories python3 train_areal.py --config config_text_rollout.yaml
```
...
python3 -m areal_pacman.dataset --mode episode --output run_artifacts/pacman_episode_legal_specs.jsonl --episodes 8 --max-steps 30 --prompt-style legal --illegal-action-penalty -4
```
...
Training uses [multi-maze GRPO config](config_vision_rollout_qwen3p5_9b_imageonly_multimaze_v1_grpo_3epoch_4sample_8gpu.yaml): `64` train mazes, `n_samples=4`, `3` epochs, `batch_size=8`, and `24` optimizer steps. Frozen base/post-RL evaluation uses [multi-maze no-train config](config_vision_rollout_qwen3p5_9b_imageonly_multimaze_v1_notrain_4sample_8gpu.yaml), with `lr=0`, `eps_clip=0`, and no actor update.

The [safe-progress config](config_vision_rollout_qwen3p5_9b_imageonly_multimaze_v1_safeprogress_grpo_3epoch_4sample_8gpu.yaml) keeps those settings fixed and adds `reward_mode=safe_progress`, `safe_progress_alpha=4.0`.

On an idle node5, run the complete guarded pipeline:

```bash
cd /home/z00819216/vla-research/areal-pacman
nohup bash scripts/remote_supervise_multimaze_v1.sh \
...
```bash
nohup bash scripts/remote_wait_for_idle_multimaze_v1.sh \
...
/home/z00819216/miniconda/envs/areal-vla/bin/python train_areal.py --config config_pacman.yaml --dry-run
...
/home/z00819216/miniconda/envs/areal-vla/bin/python train_areal.py --config config_text_rollout.yaml --dry-run
...
/home/z00819216/miniconda/envs/areal-vla/bin/python train_areal.py --config config_text_rollout_longer.yaml --dry-run
/home/z00819216/miniconda/envs/areal-vla/bin/python train_areal.py --config config_text_rollout_1p5b.yaml --dry-run
...
/home/z00819216/miniconda/envs/areal-vla/bin/python train_areal.py --config config_text_rollout_1p5b_legal.yaml --dry-run
...
/home/z00819216/miniconda/envs/areal-vla/bin/python train_areal.py --config config_text_rollout_1p5b_rules.yaml --dry-run
/home/z00819216/miniconda/envs/areal-vla/bin/python train_areal.py --config config_text_rollout_1p5b_grpo.yaml --dry-run
...
/home/z00819216/miniconda/envs/areal-vla/bin/python train_areal.py --config config_text_rollout_1p5b_overfit_grpo.yaml --dry-run
...
/home/z00819216/miniconda/envs/areal-vla/bin/python train_areal.py --config config_text_rollout_1p5b_tiny_overfit_grpo.yaml --dry-run
...
/home/z00819216/miniconda/envs/areal-vla/bin/python train_areal.py --config config_text_rollout_1p5b_teacher_overfit_grpo.yaml --dry-run
...
/home/z00819216/miniconda/envs/areal-vla/bin/python train_areal.py --config config_text_rollout_1p5b_teacher_pure_overfit_grpo.yaml --dry-run
...
PYTHONPATH=/home/z00819216/vla-research/areal-pacman:$PYTHONPATH AREAL_FORCE_HOST_IP=10.0.12.183 /home/z00819216/miniconda/envs/areal-vla/bin/python train_areal.py --config config_text_rollout_7b_medium_default_overfit_grpo_8gpu.yaml --dry-run
...
PYTHONPATH=/home/z00819216/vla-research/areal-pacman:$PYTHONPATH AREAL_FORCE_HOST_IP=10.0.12.183 /home/z00819216/miniconda/envs/areal-vla/bin/python train_areal.py --config config_text_rollout_7b_medium_default_reason_debug_8gpu.yaml --dry-run
```
...
当前 [dataset.py](areal_pacman/dataset.py) 的 system prompt 已经写明 text PacMan 规则: `#` wall, `P` PacMan, `G` ghost, `.` pellet, wall move penalty, pellet objective, ghost terminal, win condition, and one-token action output. 这是为了以后迁移到更复杂 true PacMan 时, prompt contract 可以逐步扩展, 而不是只靠 observation 里的 `Legal actions`.
...
- [config_pacman.yaml](config_pacman.yaml): 单步 smoke, 使用 baseline 生成的 `answer` 给 `0/1` reward.
- [config_text_rollout.yaml](config_text_rollout.yaml): pure text episode rollout, 不使用 ground truth answer, reward 来自 [PacmanEnv.step](areal_pacman/env.py).
- [config_text_rollout_longer.yaml](config_text_rollout_longer.yaml): 比 smoke 稍大的 text rollout, 用于检查多 episode reward 和 trajectory 分布.
- [config_text_rollout_1p5b.yaml](config_text_rollout_1p5b.yaml): 1.5B model-size ablation, 用同一套 episode 和 analyzer 对比 0.5B.
- [config_text_rollout_1p5b_legal.yaml](config_text_rollout_1p5b_legal.yaml): 1.5B legal-action ablation, 用 `prompt_style=legal` 和 `illegal_action_penalty=-4`.
- [config_text_rollout_1p5b_rules.yaml](config_text_rollout_1p5b_rules.yaml): 1.5B rules-prompt PPO baseline, 用 expanded system prompt 但不加 illegal-action penalty.
- [config_text_rollout_1p5b_grpo.yaml](config_text_rollout_1p5b_grpo.yaml): 1.5B GRPO-style ablation, same rules dataset, `n_samples=4`, group reward normalization.
- [config_text_rollout_1p5b_overfit_grpo.yaml](config_text_rollout_1p5b_overfit_grpo.yaml): 1.5B overfit gate, 4 fixed episodes, `prompt_style=ghost_legal`, `illegal_action_penalty=-4`, `total_train_epochs=8`.
- [config_text_rollout_1p5b_tiny_overfit_grpo.yaml](config_text_rollout_1p5b_tiny_overfit_grpo.yaml): 1.5B tiny sparse overfit gate, `layout_name=tiny_corridor`, `prompt_style=ghost_legal`, no teacher hint.
- [config_text_rollout_1p5b_teacher_overfit_grpo.yaml](config_text_rollout_1p5b_teacher_overfit_grpo.yaml): 1.5B teacher-hint overfit gate, 4 fixed episodes, `prompt_style=teacher_hint`, `illegal_action_penalty=-4`.
- [config_text_rollout_1p5b_teacher_pure_overfit_grpo.yaml](config_text_rollout_1p5b_teacher_pure_overfit_grpo.yaml): 1.5B pure hint-only overfit gate, 4 fixed episodes, `prompt_style=teacher_hint_pure`, `greedy=false`, `temperature=0.1`, no execution-time teacher override.
- [config_text_rollout_1p5b_long_debug_grpo.yaml](config_text_rollout_1p5b_long_debug_grpo.yaml): larger debug config, only after overfit succeeds.
- [config_text_rollout_7b_overfit_grpo_8gpu.yaml](config_text_rollout_7b_overfit_grpo_8gpu.yaml): 7B default-map sparse overfit on 8 GPUs, no teacher hint.
- [config_text_rollout_7b_medium_default_overfit_grpo_8gpu.yaml](config_text_rollout_7b_medium_default_overfit_grpo_8gpu.yaml): 7B medium-map sparse overfit on 8 GPUs, no teacher hint.

## 当前 H100 对比

| Config | Episodes | Wins | Avg reward | Illegal actions | Terminal reasons |
| --- | ---: | ---: | ---: | ---: | --- |
| [config_text_rollout_longer.yaml](config_text_rollout_longer.yaml) | 20 | 0 | -20.00 | n/a | `max_steps=20` |
| [config_text_rollout_1p5b.yaml](config_text_rollout_1p5b.yaml) | 20 | 0 | 3.65 | n/a | `caught=3`, `max_steps=17` |
| [config_text_rollout_1p5b_legal.yaml](config_text_rollout_1p5b_legal.yaml) | 20 | 0 | -10.90 | 1 | `caught=4`, `max_steps=16` |
| [config_text_rollout_1p5b_rules.yaml](config_text_rollout_1p5b_rules.yaml) | 20 | 0 | 5.20 | 350 | `caught=3`, `max_steps=17` |
| [config_text_rollout_1p5b_grpo.yaml](config_text_rollout_1p5b_grpo.yaml) | 64 | 0 | 0.30 | 148 | `caught=12`, `max_steps=52` |
| [config_text_rollout_1p5b_overfit_grpo.yaml](config_text_rollout_1p5b_overfit_grpo.yaml) | 264 | 0 | -56.51 | 3879 | `caught=6`, `max_steps=258` |
| [config_text_rollout_1p5b_tiny_overfit_grpo.yaml](config_text_rollout_1p5b_tiny_overfit_grpo.yaml) tiny sparse | 12 | 12 | 118.00 | 0 | `all_pellets=12` |
| [config_text_rollout_1p5b_teacher_overfit_grpo.yaml](config_text_rollout_1p5b_teacher_overfit_grpo.yaml) teacher-enforced | 40 | 40 | 264.00 | 0 | `all_pellets=40` |
| [config_text_rollout_1p5b_teacher_pure_overfit_grpo.yaml](config_text_rollout_1p5b_teacher_pure_overfit_grpo.yaml) pure hint-only | 12 | 12 | 264.00 | 0 | `all_pellets=12` |
| [config_text_rollout_7b_overfit_grpo_8gpu.yaml](config_text_rollout_7b_overfit_grpo_8gpu.yaml) default sparse 8GPU | 256 | 0 | -78.93 | 3569 | `max_steps=256` |
| [config_text_rollout_7b_medium_default_overfit_grpo_8gpu.yaml](config_text_rollout_7b_medium_default_overfit_grpo_8gpu.yaml) medium sparse 8GPU | 207 | 0 | -78.91 | 2541 | `max_steps=207` |
